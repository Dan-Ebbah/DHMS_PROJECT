...
public class SHEHospital implements Hospital {

    private static final int MY_UDP_PORT_1 = 11003;
    private static final int MY_UDP_PORT_2 = 11006;
    private static final int QUE_UDP_PORT_2 = 11005;
    private static final int MTL_UDP_PORT_2 = 11004;
    ...
    public SHEHospital() throws SocketException {
        ...
        this.hospitalUtil = new HospitalUtil();
    ...
    public String bookAppointment(String patientID, String appointmentID, String appointmentType) {
        ...
        Hospital mtlHospital = hospitalUtil.getHospital("MTL");

        if (queHospital == null || mtlHospital == null) {
            logStatus = "Failure";
        ...
        if (!patientID.startsWith("SHE")) {
            ...
            } else if (patientID.startsWith("QUE")) {
                appointmentCountInOtherHospitals += mtlHospital.countOfAppointmentsInSameWeek(appointmentID, patientID);
            }
        ...
        if (appointmentWithSameTypeOnSameDayExists(appointmentType, appointmentID, patientID)
                || mtlHospital.appointmentWithSameTypeOnSameDayExists(appointmentType, appointmentID, patientID)
    ...
    public String getAppointmentSchedule(String patientID) {
        ...
        StringBuffer appointmentSchedule = new StringBuffer(
                this.getAppointmentsOfPatient(patientID)
                        ...
                        + mtlHospital.getAppointmentsOfPatient(patientID));
    ...
    public String cancelAppointment(String patientID, String appointmentID) {
        ...
        if (appointmentID.startsWith("QUE")) {
            ...
            return mtlHospital.cancelAppointment(patientID, appointmentID);
        }
    ...
    public String listAppointmentAvailability(String type) {
        ...
        Thread t1 = new Thread(() -> hospitalUtil.findAvailabilityIn("MTL", availabilityList, type));
        Thread t2 = new Thread(() -> hospitalUtil.findAvailabilityIn("QUE", availabilityList, type));
        Thread t3 = new Thread(() -> hospitalUtil.findAvailabilityInThisCity("SHE", availabilityList, type, appointments));
    ...
    public String getAppointmentsOfPatient(String patientID) {
        ...
        appointments.forEach((appointmentType, stringAppointmentDetailsHashMap) ->
                stringAppointmentDetailsHashMap.forEach((appointmentID, appointmentDetails) -> {
                    if (appointmentDetails.getPatientIDList().contains(patientID)) {
                        appointmentIdsOfPatient.computeIfAbsent("SHE", k -> new HashMap<>());
                        appointmentIdsOfPatient.get("SHE").computeIfAbsent(appointmentType, k -> new ArrayList<>());
                        appointmentIdsOfPatient.get("SHE").get(appointmentType).add(appointmentID);
                    }
                }));
        ...
        if (!appointmentIdsOfPatient.isEmpty()) {
            appointmentIdsOfPatient.get("SHE").forEach((type, idList) -> {
